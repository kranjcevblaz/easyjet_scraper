...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 20 00:15:03 2021
...
# =============================================================================
headless_chrome = True # set to False to get browser UI (runs a bit slower but you see it running in separate browser window)
...
def get_airport_code_from_name():
    ''' func scrapes and cleans airport codes from Timetables page and assigns to dict of format {'airport_name':'IATA_airport_code'}'''
    ...
    departure_airport_dict = {departure_airport_code[i]: departure_airport_name[i] for i in range(len(departure_airport_code))}
...
def get_airport_code_country():
    ...
    dropdown_content = sel_soup.find_all('div', {'class': 'lff-filter-column-content'})[0].find_all('div', {'class': 'lff-filter-item'})
    driver.close()
    ...
    airport_code_country_dict = {airport_code_list[i]: airport_code_country_list[i] for i in range(len(airport_code_list))}
    ...
    for arr_airport_name in airport_code_list:
        for k,v in departure_airport_dict.items():
            if v == arr_airport_name:
                arr_airport_code = k
        url = "https://www.easyjet.com/ejcms/nocache/jscallbacks/TimetableCallback.aspx?data={{%22languageCode%22:%22en%22,%22originIata%22:%22{0}%22,%22destinationIata%22:%22{1}%22,%22passengers%22:1}}".format(dep_airport_code, arr_airport_code)
        print(url) # useful for debugging the JSON objects urls
...
def get_flight_names(json_data):
    ...
    for day in json_access_outbound:
        day_access = day['days']
        for flight in day_access:
            flight_access = flight['flights']
            for flight_dict in flight_access:
                if flight_dict != None:
                    save_data_outbound.append(flight_dict)
                    
    for day in json_access_return:
        day_access = day['days']
        for flight in day_access:
            flight_access = flight['flights']
            for flight_dict in flight_access:
                if flight_dict != None:
                    save_data_return.append(flight_dict)
                    
    for item in save_data_outbound:
        ...
        outbound_return_list.append('OutboundLeg')
        
    for item in save_data_return:
        flight_id_list.append(item['id'])
        flight_num_list.append(item['flightNum'])
        localDepTime_list.append(item['localDepTime'])
        localArrTime_list.append(item['localArrTime'])
        price_list.append(item['price'])
        currency_list.append('GBP')
        outbound_return_list.append('ReturnLeg')
        
    flight_info_list = [list(i) for i in zip(flight_id_list, flight_num_list, localDepTime_list, localArrTime_list, price_list, currency_list, outbound_return_list)]
    return flight_info_list

def transform_dataframe(df):
    df.columns = ['flight_id', 'flight_number', 'LocalDepTime', 'LocalArrTime', 'price', 'currency', 'direction', 'from_airport_city', 'from_airport_code', 'to_airport_city', 'to_airport_code', 'from_country', 'to_country']
    ...
    # m = df['direction'] == 'returnLeg'
    df['from_airport_code'],df['to_airport_code']=np.where(df['direction']=='ReturnLeg',(df['to_airport_code'],df['from_airport_code']),(df['from_airport_code'],df['to_airport_code']))
    df['from_airport_city'],df['to_airport_city']=np.where(df['direction']=='ReturnLeg',(df['to_airport_city'],df['from_airport_city']),(df['from_airport_city'],df['to_airport_city']))

    df.drop('direction', 1, inplace=True)
    # mp = {'from_airport_code': 'to_airport_code', 'to_airport_code': 'from_airport_code'}
    # df.update(df.loc[m].rename(mp, axis=1))

    # df = df.loc[m, ['from_airport_code', 'to_airport_code']] = (df.loc[m, ['to_airport_code', 'from_airport_code']].values)
    # df = df.loc[m, ['from_airport_city', 'to_airport_city']] = (df.loc[m, ['to_airport_city', 'from_airport_city']].values)
    return df
...
if run_from_editor:
    ...
    parser = argparse.ArgumentParser(description='Scrape EasyJet flights based on departure airports and number of months')
    parser.add_argument('-d', '--airports', nargs='+', help='list of departure airport codes', required=True, default=[])
